...
disp('    Continuous-time SRIVC-based method for Transfer Function Models')
disp('     (TFFSRIVC).')
disp('   ')
disp('    The TFFSRIVC method is used here to estimate a continuous-time');
...
%   object with the TFFSRIVC algorithm. The extra information
%   required are the number of poles and zeros, i.e. np and nz.

    nz = length(num) - 1;
    np = length(den)-1;
    
    M = tffsrivc(data,np,nz)
...
%   model by using the command STEP.
    ...
    figure
    step(M0,M)
    legend('True model','Estimated by TFFSRIVC')
...
%   Hit any key
    ...
    clc    
%   See the help of TFFRIVC for more explanations. See also TFFCOE
